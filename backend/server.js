...
// Helper function to run queries (compatible with better-sqlite3 API)
...
app.get('/api/profile', (req, res) => {
  try {
    ...
    let profile = db.prepare('SELECT * FROM user_profiles WHERE session_id = ?').get(sessionId);

    if (!profile) {
      // Create new profile
      const stmt = db.prepare('INSERT INTO user_profiles (session_id) VALUES (?)');
      const result = stmt.run(sessionId);
      profile = db.prepare('SELECT * FROM user_profiles WHERE id = ?').get(result.lastInsertRowid);
    }

    // Get user info if exists
    const userInfo = db.prepare('SELECT * FROM user_info WHERE profile_id = ?').get(profile.id);
...
app.post('/api/interaction', (req, res) => {
  try {
    ...
    // Get profile
    const profile = db.prepare('SELECT * FROM user_profiles WHERE session_id = ?').get(sessionId);
