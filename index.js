...
app.event("team_join", async ({ event, client, logger }) => {
  if (!welcomeChannelId) {
    logger.error("Welcome channel not configured");
    return;
  }
...
app.message(/^help$/i, async ({ say }) => {
  await say({
    text:
      `🤖 *Keystone Bot - Available Commands*\n` +
...
app.event("message", async ({ event, say }) => {
  ...
  try {
    ...
    if (!profile) {
      ...
      await say(
        "✅ Your profile is already set up! Ask me questions by mentioning me in a channel.",
      );
    }
  } catch (error) {
...
app.view("intake_submission", async ({ ack, body, view, client }) => {
  ...
  const interests =
    values.interests_block?.interests_select?.selected_options?.map(
      (opt) => opt.value,
    ) || [];
  const complexity =
    values.complexity_block?.complexity_select?.selected_option?.value || "";
  const data = {
    name,
    email,
    role,
    ...
    blocks: [
      ...
      },
      {
        ...
        element: {
          ...
          options: [
            {
              text: { type: "plain_text", text: "Frontend Development" },
              value: "frontend",
            },
            {
              text: { type: "plain_text", text: "Backend Development" },
              value: "backend",
            },
            {
              text: { type: "plain_text", text: "Database Design" },
              value: "database",
            },
            {
              text: { type: "plain_text", text: "API Development" },
              value: "api",
            },
            { text: { type: "plain_text", text: "Testing" }, value: "testing" },
            { text: { type: "plain_text", text: "DevOps" }, value: "devops" },
            {
              text: { type: "plain_text", text: "UI/UX Design" },
              value: "design",
            },
            {
              text: { type: "plain_text", text: "System Architecture" },
              value: "architecture",
            },
            {
          ...
          options: [
            {
              text: {
                type: "plain_text",
                text: "Beginner - Simple explanations",
              },
              value: "beginner",
            },
            {
              text: {
                type: "plain_text",
                text: "Intermediate - Balanced detail",
              },
              value: "intermediate",
            },
            {
              text: { type: "plain_text", text: "Advanced - Technical depth" },
              value: "advanced",
            },
          ],
        },
...
(async () => {
  ...
  const channel = result.channels.find(
    (candidate) => candidate.name === "all-project-keystone",
  );
